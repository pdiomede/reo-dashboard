...
|----------|---------|----------|--------|
| `telegram_bot.log` | Technical bot operations | `/home/graph/ftpbox/reo/` | Timestamped with log levels |
| `telegram_bot_activity.log` | User activity tracking | `/home/graph/ftpbox/reo/` | Timestamped events |
...
| `subscribers.json` | Subscriber database | `/home/graph/ftpbox/reo/` | JSON with timestamps |
...
**Last 100 lines:**
```bash
tail -n 100 /home/graph/ftpbox/reo/telegram_bot.log
```

**Search for errors:**
```bash
grep ERROR /home/graph/ftpbox/reo/telegram_bot.log
```

**View specific date:**
```bash
grep "2025-10-22" /home/graph/ftpbox/reo/telegram_bot.log
```
...
**Live monitoring:**
```bash
tail -f /home/graph/ftpbox/reo/telegram_bot_activity.log
```

**See all new subscribers:**
```bash
grep "NEW_SUBSCRIBER" /home/graph/ftpbox/reo/telegram_bot_activity.log
```

**See all unsubscribes:**
```bash
grep "UNSUBSCRIBED" /home/graph/ftpbox/reo/telegram_bot_activity.log
```

**Count new subscribers today:**
```bash
grep "NEW_SUBSCRIBER" /home/graph/ftpbox/reo/telegram_bot_activity.log | grep "$(date +%Y-%m-%d)" | wc -l
```

**See all bot startups:**
```bash
grep "BOT STARTED" /home/graph/ftpbox/reo/telegram_bot_activity.log
```

**Last 50 activity events:**
```bash
tail -n 50 /home/graph/ftpbox/reo/telegram_bot_activity.log
```

**Activity for specific user (by chat ID):**
```bash
grep "123456789" /home/graph/ftpbox/reo/telegram_bot_activity.log
```

**Activity for specific username:**
```bash
grep "@alice_indexer" /home/graph/ftpbox/reo/telegram_bot_activity.log
```
...
**Pretty print:**
```bash
cat /home/graph/ftpbox/reo/subscribers.json | python3 -m json.tool
```
...
**View stats only:**
```bash
cat /home/graph/ftpbox/reo/subscribers.json | grep -A3 '"stats"'
```
...
grep ERROR /home/graph/ftpbox/reo/telegram_bot.log | tail -n 10

# Check subscriber count
cat /home/graph/ftpbox/reo/subscribers.json | grep total_subscribers
```
...
grep "NEW_SUBSCRIBER" /home/graph/ftpbox/reo/telegram_bot_activity.log | grep "$(date +%Y-%m)" | wc -l

# Count unsubscribes this week
grep "UNSUBSCRIBED" /home/graph/ftpbox/reo/telegram_bot_activity.log | grep "$(date +%Y-%m)" | wc -l
...
# Check subscriber count
SUBS=$(cat /home/graph/ftpbox/reo/subscribers.json | grep total_subscribers | grep -o '[0-9]*')
echo "👥 Active Subscribers: $SUBS"

echo ""

# Check notifications sent
NOTIFS=$(cat /home/graph/ftpbox/reo/subscribers.json | grep total_notifications_sent | grep -o '[0-9]*')
echo "📤 Total Notifications Sent: $NOTIFS"

echo ""

# Check for recent errors
ERROR_COUNT=$(grep ERROR /home/graph/ftpbox/reo/telegram_bot.log | wc -l)
...
tail -n 5 /home/graph/ftpbox/reo/telegram_bot_activity.log
...
if [ -f telegram_bot.log ]; then
    mv telegram_bot.log telegram_bot.log.$DATE
    touch telegram_bot.log
fi

if [ -f telegram_bot_activity.log ]; then
    mv telegram_bot_activity.log telegram_bot_activity.log.$DATE
    touch telegram_bot_activity.log
fi
...
find /home/graph/ftpbox/reo -name "*.log.*" -mtime +30 -delete
...
tail -f /home/graph/ftpbox/reo/telegram_bot.log           # Technical
tail -f /home/graph/ftpbox/reo/telegram_bot_activity.log  # Activity
tail -f /home/graph/ftpbox/reo/cron.log                   # Dashboard
sudo journalctl -u telegram_bot.service -f                 # Systemd

# Search logs
grep "NEW_SUBSCRIBER" telegram_bot_activity.log    # New subs
grep "UNSUBSCRIBED" telegram_bot_activity.log      # Unsubs
grep ERROR telegram_bot.log                         # Errors
grep "notifications sent" cron.log                  # Notif stats

# Count events
grep "NEW_SUBSCRIBER" telegram_bot_activity.log | wc -l      # Total subs
grep "UNSUBSCRIBED" telegram_bot_activity.log | wc -l        # Total unsubs
grep "BOT STARTED" telegram_bot_activity.log | wc -l         # Bot restarts

# View specific user activity
grep "123456789" telegram_bot_activity.log         # By chat ID
grep "@username" telegram_bot_activity.log         # By username

# Check current status
ps aux | grep telegram_bot.py                      # Is running?
cat subscribers.json | grep total_subscribers      # Sub count
tail -n 1 telegram_bot_activity.log                # Last event
```
...
1. `telegram_bot_activity.log` - **Most important for tracking subscribers**
2. `telegram_bot.log` - Technical issues and errors
3. `cron.log` - Dashboard script and notification delivery
4. `subscribers.json` - Current subscriber database

All logs are automatically created and maintained. No manual intervention needed! 📊
